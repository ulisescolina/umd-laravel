...
if [ "${1}" == "php" -a "$2" == "artisan" -a "$3" == "serve" ]; then
  if ! app_present; then
    log "No se puede iniciar el contenedor con una aplicación Laravel desde cero."
    log "Debe iniciarla desde un proyecto que ya existente."
    exit 1
  fi

  if composer_json_present ; then
    ...
    log "No cuenta con un archivo composer.json"
    log "Esto puede significar que:"
    log "- El proyecto no tiene dependencias"
    log "- No tiene clonado el repositorio"
    exit 1
  fi
